...
public partial class QuanLyKhuyenMaiViewModel : ObservableObject
{
    ...
    public string Status => EndDate.Date >= DateTime.Today && StartDate.Date <= DateTime.Today ? "Dang ap dung" : "Da ket thuc";
    ...
    [RelayCommand] private async Task SaveAsync(){try{var r=new YeuCauKhuyenMai(Name,Type=="PHAN_TRAM"?Value:null,DateOnly.FromDateTime(StartDate),DateOnly.FromDateTime(EndDate),Condition,Type,Type=="PHAN_TRAM"?null:Value,null);if(SelectedPromotion is null)await _api.CreatePromotionAsync(r);else await _api.UpdatePromotionAsync(SelectedPromotion.MaKM,r);Clear();await LoadAsync();}catch(LoiYeuCauApi e){Message=e.Message;}}
